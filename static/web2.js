...
const text_download_info = {script: "", text: "", percent: "", text_algo: ""}
...
text_input.addEventListener("input", buttonUpdate)
...
function buttonUpdate() {
    ...
    if(url_input.value.length <= 0 && text_input.value.length <= 0){
        summarize_button.disabled = true;
    }
    else{
        if(percent_dropdown.selectedIndex <= 0 || choice_dropdown.selectedIndex <= 0){
            summarize_button.disabled = true;
        }
        else{
    ...
    // if(url_input.value.length > 0 && percent_dropdown.selectedIndex > 0 && choice_dropdown.selectedIndex > 0){
    //     summarize_button.disabled = false;
    // }
    // else if(text_input.value.length > 0 && percent_dropdown.selectedIndex > 0 && choice_dropdown.selectedIndex > 0){
    //     summarize_button.disabled = false;
    // }
    // else{
    //     summarize_button.disabled = true;
...
function initializeSummary() {
    ...
    if (percent_dropdown.selectedIndex > 0 && choice_dropdown.selectedIndex > 0 && url_input.value.length > 0) {
        ...
        if (video_id) {
            // text_out_content_element.innerHTML = "viseo successfully taken";
            ...
            fetch("/summarize/?id=" + video_id +
                ...
                if (result.success) {
                    ...
                    text_out_content_element.innerHTML = "<div style='color: white'><b>Generated Summary:</b> " + response_json.processed_summary
                        + "<p>In your video, there are <b>" + response_json.length_original + "</b> characters."
                        + "<br>The processed summary has <b>" + response_json.length_summary + "</b> characters"
                        + "</br><br></div>";
    ...
    else if(percent_dropdown.selectedIndex > 0 && choice_dropdown.selectedIndex > 0 && text_input.value.length > 0){
        const text_out_content_element = document.getElementById("text-out");
        const process_element = document.getElementById("current_process");
        ...
        // const text = parse_youtube_video_id(url);
        const percent = percent_value.split("%")[0];
        const choice = parse_choice(choice_index);

        fetch("/text_summarize/?text_input=" + text +
            "&percent=" + percent + "&choice=" + choice)
            .then(response => response.json()).then(result => {
            // Result now contains the response in JSON
            // Sending result back to popup.html
            ...
            if (result.success) {
                ...
                text_out_content_element.innerHTML = "<b>Processed Summary:</b> " + response_json.processed_summary
                    + "<p>The generated summary is"
                    + "<p>In your video, there are <b>" + response_json.length_original + "</b> characters "
                    + "<br>The processed summary has <b>" + response_json.length_summary + "</b> characters"
                    + "</br><br>";

                // Populating the globally created dictionary
                // text_download_info.script = response_json.processed_summary
                // text_download_info.text_input = text_input
                // text_download_info.text_algo = choice.replaceAll('-', '_')
                // text_download_info.percent = percent
                // // Displaying download button
                // download_button.style.display = "block";
                // // Text Beautification: Aligning Text to be justified
                // text_out_content_element.style.textAlign = "justify";
...
// function downloadTextSummaryScript() {
//     // Creating hyperlink element with tagName "a"
//     var element = document.createElement('a');
//     // Setting HREF to the script text
//     element.setAttribute('href', 'data:application/octet-stream; data:text/plain;charset=utf-8,' +
...
//     // Setting download file name in the format "script_videoID_algo_percent.txt"
//     element.setAttribute('download', "script_" +
//         download_info.text_input + "_" + download_info.text_algo + "_" + download_info.percent + ".txt");
//     // Making the hyperlink invisible and appending it to the body
//     element.style.display = 'none';
//     document.body.appendChild(element);
